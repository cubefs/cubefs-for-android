...
func mountDirect(mountPoint string, opts *MountOptions, ready chan<- error) (fd int, err error) {
	if !opts.NoEmpty { // mount point must be empty
		canMount(mountPoint)
	}
	...
	if err != nil {
		syscall.Close(fd)
		return 0, fmt.Errorf("mount failed, err %s", err.Error())
	}
...
func canMount(mountPoint string) {
	dirs, err := ioutil.ReadDir(mountPoint)
	if err != nil || len(dirs) != 0 {
		log.Fatalf("mount point %s may be not empty, fliles %d, err %v", mountPoint, len(dirs), err)
	}

	shell := fmt.Sprintf("mount | grep -w '%s'", mountPoint)
	cmd := exec.Command("/bin/sh", "-c", shell)
	result, _ := cmd.Output()

	if result != nil && len(result) != 0 {
		log.Fatalf("mount ponit %s may be already mounted", mountPoint)
	}
