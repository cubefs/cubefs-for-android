...
func (ms *Server) handleRequest(req *request) Status {
	...
	if req.status.Ok() && ms.opts.Debug {
		fuselog.FuseLog.Debug("in", zap.String("msg", req.InputDebug()))
	}
	...
	if req.inHeader.NodeId == pollHackInode ||
		...
		fuselog.FuseLog.Debug("unimplemented opcode ", zap.Any("op", operationName(req.inHeader.Opcode)))
		req.status = ENOSYS
	...
	errNo := ms.write(req)
	if errNo != 0 {
		fuselog.FuseLog.Warn("write/writev failed",
			zap.Any("op", operationName(req.inHeader.Opcode)),
			zap.Any("err", errNo.String()))
	}
...
func (ms *Server) write(req *request) Status {
	...
	if ms.opts.Debug {
		fuselog.FuseLog.Debug("out", zap.String("msg", req.OutputDebug()))
	}
